...
//start replaced branch: ENGDEVEMSEFee:CD_Deposit
{
	...
	if (feeExists('CD001', 'NEW') && calcValue > 100000) {
		if (totalValCalUnits >= 1000 && totalValCalUnits <= 9999)
			feeAmount = 5000;
	}

	if (feeExists('CD001', 'NEW') && calcValue > 100000) {
		if (totalValCalUnits >= 10000 && totalValCalUnits <= 19999)
			feeAmount = 10000;
	}

	if (feeExists('CD001', 'NEW') && calcValue > 100000) {
		if (totalValCalUnits >= 20000)
			feeAmount = 15000;
	}
...
//start replaced branch: ENGDEVEMSEFee:CD_Admin
{
	...
	if (feeExists('CD002', 'NEW') && calcValue > 100000) {
		if (totalValCalUnits >= 5000 && totalValCalUnits <= 9999)
			feeAmount = 500;
	}
