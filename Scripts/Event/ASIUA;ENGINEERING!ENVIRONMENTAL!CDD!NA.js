...
if (typeof(DIVERSIONPROGRAMINFORMATION) == 'object') {
	...
	//start replaced branch: ENG:CDDCALCTOTALS
	{
		if (typeof(DIVERSIONPROGRAMINFORMATION) == 'object') {
			diversionTot = 0;
			for (xxx in DIVERSIONPROGRAMINFORMATION)
				if (DIVERSIONPROGRAMINFORMATION[xxx]['Material Type'] != 'Trash' && DIVERSIONPROGRAMINFORMATION[xxx]['Ticket Status'] == 'Approved')
					diversionTot += DIVERSIONPROGRAMINFORMATION[xxx]['Net Weight in Tons'] * 1;
		...
		if (typeof(DIVERSIONPROGRAMINFORMATION) == 'object') {
			...
			for (xxx in DIVERSIONPROGRAMINFORMATION)
				if (DIVERSIONPROGRAMINFORMATION[xxx]['Material Type'] == 'Trash' && DIVERSIONPROGRAMINFORMATION[xxx]['Ticket Status'] == 'Approved')
					refuseTot += DIVERSIONPROGRAMINFORMATION[xxx]['Net Weight in Tons'] * 1;
