if (appTypeArray[0] != 'Engineering' && !(matches(appTypeArray[1], 'CIP Permit', 'LCP', 'OLP', 'ROW'))) {
	//start replaced branch: ASA:BuildingViolation:CodeViolation:FireDistrict:Check
	{
		...
		if (existFDRecArray) {
			for (x1 in existFDRecArray)
				if (!matches(aa.cap.getCap(existFDRecArray[x1].getCapID()).getOutput().getCapStatus(), 'Void', 'No Violations', 'Closed'))
					fdRecExist = true;
		}
		...
		if (existEnfRecArray) {
			for (x2 in existEnfRecArray)
				if (!matches(aa.cap.getCap(existEnfRecArray[x2].getCapID()).getOutput().getCapStatus(), 'Void', 'Pending', 'Closed'))
					enfRecExist = true;
		...
		if (existBldRecArray) {
			for (x3 in existBldRecArray)
				if (!matches(aa.cap.getCap(existBldRecArray[x3].getCapID()).getOutput().getCapStatus(), 'Resolved', 'Closed-Duplicate', 'Closed-Not Within City', 'Closed'))
					fdRecExist = true;
		}
