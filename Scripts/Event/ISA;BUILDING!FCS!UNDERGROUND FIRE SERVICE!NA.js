...
if (!publicUser && inspType != lookup('EMSE:NONBLDGINSPTYPES', inspType) && (matches(inspGroup, 'FCS_UG','FCS_FIXED', 'FCS_HGHPLE')) && matches(inspInspector, '', ' ')) {
	assignInspection(inspId, lookup('EMSE:FDINSPECTOR:LOOKUP', fireInspDist));
}

if (publicUser && inspType != lookup('EMSE:NONBLDGINSPTYPES', inspType) && (matches(inspGroup, 'FCS_UG','FCS_FIXED', 'FCS_HGHPLE'))) {
	assignInspection(inspId, lookup('EMSE:FDINSPECTOR:LOOKUP', fireInspDist));
}