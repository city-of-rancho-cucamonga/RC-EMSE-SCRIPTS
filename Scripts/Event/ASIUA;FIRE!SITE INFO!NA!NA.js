...
vFeeExists=false;
for (xx in PERMITS) if(PERMITS[xx]['Fee Item Seq']==vFeeItemSeq) vFeeExists=true;
for (xx in INSPECTIONTYPES) if(INSPECTIONTYPES[xx]['Fee Item Seq']==vFeeItemSeq) vFeeExists=true;
for (xx in FEESANDCHARGES) if(FEESANDCHARGES[xx]['Fee Item Seq']==vFeeItemSeq) vFeeExists=true;
...
if (!feeExists(lookup('FD Permit',permitDesc),'NEW','INVOICED')) {
	...
	if(feeItemSeq) editFirePermitsASITableRow(capId, 'PERMITS', 'Permit Desc', permitDesc, 'Fee Item Seq', feeItemSeq.toString(),false);
	}
...
if (!feeExists(lookup('FD_FLDINSPTYPE',INSPECTIONTYPES[x]['Inspection Type'],'NEW','INVOICED'))) {
	...
	if(feeItemSeq) editFirePermitsASITableRow(capId, 'INSPECTION TYPES', 'Inspection Type', inspType.toString(), 'Fee Item Seq', feeItemSeq.toString(),false);
	}
...
for (x in PERMITS) feeRecTotalNew += feeAmount(lookup('FD Permit',PERMITS[x]['Permit Desc']), 'NEW');
comment('What is Fee Record Total for NEW $ ' + feeRecTotalNew);
for (x in INSPECTIONTYPES) feeRecTotalNew += feeAmount(lookup('FD_FLDINSPTYPE',INSPECTIONTYPES[x]['Inspection Type']), 'NEW');
comment('What is Fee Record Total for NEW $ ' + feeRecTotalNew);
if (feeRecTotalNew > 0) {
	for (x in REDUCTIONS) if (matches (REDUCTIONS[x]['Description'], 'Fee Waived per District Policy')) isFeeWaived = true;
	}

if (feeRecTotalNew > 0) {
	for (x in REDUCTIONS) if (matches (REDUCTIONS[x]['Description'], 'Non-Profit Organization Fee Reduction')) isNonProfit = true;
	}
...
if (!feeExists(lookup('FD_FEESANDCHARGES',FEESANDCHARGES[x]['Charge'],'NEW','INVOICED'))) {
	...
	if(feeItemSeq) editFirePermitsASITableRow(capId, 'FEES AND CHARGES', 'Charge', charge.toString(), 'Fee Item Seq', feeItemSeq.toString(),false);
	}
