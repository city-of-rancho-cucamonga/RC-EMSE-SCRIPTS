...
if (wfTask == 'Permit Issuance' && wfStatus == 'Issued' && recordParents != null) {
	for (recCnt in recordParents)
		if (aa.cap.getCapDetail(recordParents[recCnt]).getOutput().getBalance() > 0)
			parentBal = aa.cap.getCapDetail(recordParents[recCnt]).getOutput().getBalance();
}
