...
if (typeof(SIGNINFOPERM) == 'object') {
	...
	for (x in SIGNINFOPERM)
		if (matches(SIGNINFOPERM[x]['Type'], 'Wall New', 'Wall Copy Change'))
			totSgnAreaWall += SIGNINFOPERM[x]['Square Footage'] * 1;
}
...
if (typeof(SIGNINFOPERM) == 'object') {
	...
	for (x in SIGNINFOPERM)
		if (matches(SIGNINFOPERM[x]['Type'], 'Monument New', 'Monument Copy Change'))
			totSgnAreaMonument += SIGNINFOPERM[x]['Square Footage'] * 1;
}
