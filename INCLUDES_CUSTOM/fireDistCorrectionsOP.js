function fireDistCorrectionsOP() {
	...
	if (ca.length > 0)
		for (var i in ca) {
			...
			if (c["Send Email"] == "CHECKED") {
				...
				sendReportInEmail(capId, capId.getCustomID(), "Operating Info for Email", hm, "Fire", "ADMIN", efrom, c["Email Address"], ecc, esub, ebody, false);
			}
		}
	...
	if (save2edms) {
		...
		var hm = aa.util.newHashMap()
			hm.put("altId", capId.getCustomID() + "");
		sendReportInThread(capId, capId.getCustomID(), "Operating Info", hm, "Fire", "ADMIN", efrom, "", "", esub, ebody, save2edms, false);
	}
