function bldSepticFee() {
	...
	//start replaced branch: BuildingSepticFee:Lookup
	{
		if (typeof(SEPTICINFORMATIONDETAIL) == 'object') {
			for (xxx in SEPTICINFORMATIONDETAIL)
				if (SEPTICINFORMATIONDETAIL[xxx]['Type of Use'] == 'Private Sewage Disposal System (new/altered) - Residential')
					newSepticTotal += parseFloat(SEPTICINFORMATIONDETAIL[xxx]['Quantity']) * 394.00;
		}
	...
	if (newSepticTotal > 0) {
		updateFee('SE001', 'BLD_SEPTIC', 'FINAL', newSepticTotal*1.026*1.029*1.023, 'N');
		updateFee('SE002', 'BLD_SEPTIC', 'FINAL', newSepticTotal*1.026*1.029*1.023, 'N');
