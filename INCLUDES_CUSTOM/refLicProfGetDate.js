function refLicProfGetDate(pLicNum, pDateType) {
	...
	//Internal Functs needed: convertDate(), jsDateToMMDDYYYY()
	...
	if (pDateType == null || pDateType == "")
		...
		if (!(dateType == "ISSUE" || dateType == "RENEW" || dateType == "BUSINESS" || dateType == "INSURANCE" || dateType == "WORKCOMP"))
			dateType = "EXPIRE";
	}
	...
	var newLic = getRefLicenseProf(pLicNum)
		...
		if (newLic) {
			...
			} else if (dateType == "WORKCOMP") {
				if (newLic.getWcExpDate()) {
					jsExpDate = convertDate(newLic.getWcExpDate());
					logDebug(pLicNum + " License Worker's Comp Cancel Date: " + jsDateToMMDDYYYY(jsExpDate));
					return jsExpDate;
				} else {
					logDebug("Reference record for license " + pLicNum + " has no Worker's Comp Canc Date");
					return ("NO DATE FOUND");
				}
