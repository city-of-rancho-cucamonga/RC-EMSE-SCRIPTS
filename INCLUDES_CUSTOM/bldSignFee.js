...
if (typeof(SIGNTYPE) == 'object') {
	for (xxx in SIGNTYPE) if (SIGNTYPE [xxx]['Type'] == 'Blade, Channel Letter, Directional (structural)') newSignTotal += parseFloat(SIGNTYPE [xxx]['Quantity']) * 341.00;
	}

if (typeof(SIGNTYPE) == 'object') {
	for (xxx in SIGNTYPE) if (SIGNTYPE [xxx]['Type'] == 'Monument (structural and electrical)') newSignTotal += parseFloat(SIGNTYPE [xxx]['Quantity']) * 499.00;
	}

if (typeof(SIGNTYPE) == 'object') {
	for (xxx in SIGNTYPE) if (SIGNTYPE [xxx]['Type'] == 'Wall (structural and electrical)') newSignTotal += parseFloat(SIGNTYPE [xxx]['Quantity']) * 407.00;
	}
