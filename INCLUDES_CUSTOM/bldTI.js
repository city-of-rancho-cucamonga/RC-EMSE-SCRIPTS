function bldTI() {
	...
	if (typeof(AREAINFORMATION) == 'object') {
		for (eachrow in AREAINFORMATION)
			//start replaced branch: BuildingTI:Lookup
		{
			...
			// DISABLED: BuildingTI:Lookup:z20
			// theValue = feeRow['IBC Class'] + '-' + feeRow['IBC Occupancy Type'];
			// DISABLED: BuildingTI:Lookup:z30
			// theFeeTable = 'BLDTI_Fee2013';

		}
		//end replaced branch: BuildingTI:Lookup;
	}

	// DISABLED: BLDEMSEFee:BuildingTI:30
	// if (newTotal > 0) {
	...
	if (newTotal > 0) {
		updateFee('TI012', 'BLD_TI', 'FINAL', newTotal*1.026*1.029*1.023, 'N');
		updateFee('TI014', 'BLD_TI', 'FINAL', newTotal*1.026*1.029*1.023, 'N');
		updateFee('TI090', 'BLD_TI', 'FINAL', newTotal, 'N');
